...
# from mmcv1.runner import auto_fp16
...
class BiFPNModule(nn.Module):
    def __init__(self,
        ...
        for jj in range(2):  # 0,1
            for i in range(self.levels-1):  # 0,1,2
                ...
                self.bifpn_convs.append(fpn_conv)  # 6fpn_conv
    ...
    def forward(self, inputs):
        ...
        w1 = w1_relu / (torch.sum(w1_relu, dim=0) + self.eps)  # normalize
        w2_relu = self.relu2(self.w2)
        w2 = w2_relu / (torch.sum(w2_relu, dim=0) + self.eps)  # normalize
        # build top-down
        idx_bifpn = 0
        pathtd = inputs
        inputs_clone = []
